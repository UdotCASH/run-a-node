...
if [ "$1" = "run" ]; then
    if [ "$2" = "geth" ]; then
        if [ "$3" = "1" ]; then
            # Run geth command 1
            cd devnet &&
	    ./geth --http --http.api eth,net,web3,admin,txpool --ws --ws.api eth,net,web3 --authrpc.jwtsecret jwt.hex --datadir gethdata --syncmode full --allow-insecure-unlock --unlock 0x123463a4b065722e99115d6c222f267d9cabb524
